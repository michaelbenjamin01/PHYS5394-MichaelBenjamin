...
figure;
plot(timeVec,sigVec);
title('exponentially damped sinusoid timeseries, sampling frequency fs = 5*nqstfs')
xlabel('time');
ylabel('amplitude');
grid on; grid minor;
...
title('exponentially damped sinusoid periodogram, sampling frequency fs = 5*nqstfs');
xlabel('Positive Frequencies');
ylabel('Amplitude');
grid on; grid minor;

% plotting spectrogram
winLen = 0.3; % in seconds
ovrlp = 0.1; % in seconds
...
title('exponentially damped sinusoid spectrogram, sampling frequency fs = 5*nqstfs');
xlabel('Time (sec)');
ylabel('Frequency (Hz)');
...
figure;
plot(timeVec,sigVec);
title('exponentially damped sinusoid timeseries, sampling frequency fs = 1/2*nqstfs')
grid on; grid minor;
...
figure;
plot(posFreq,abs(fftSig));
title('exponentially damped sinusoid periodogram, sampling frequency fs = 1/2*nqstfs')
grid on; grid minor;

% plotting spectrogram
winLen = 0.4; % in seconds
ovrlp = 0.3; % in seconds
...
figure;
imagesc(T,F,abs(S)); axis xy;
title('exponentially damped sinusoid spectrogram, sampling frequency fs = 1/2*nqstfs');
xlabel('Time (sec)');
ylabel('Frequency (Hz)');